export default (function (root, factory) {
  ...
  // Notific: Variables: end

  // Notific: Notify Default Settings: begin
  var notifySettings = {
    wrapID: 'NotificNotifyWrap', // can not customizable
    width: '280px',
  ...
  // Notific: Notify Default Settings: end

  // Notific: Report Default Settings: begin
  var reportSettings = {
    ID: 'NotificReportWrap', // can not customizable
    className: 'Notific-report',
    width: '320px',
    backgroundColor: 'bg-[var(--goat-black)]',
    borderRadius: '25px',
    rtl: false,
    zindex: 4002,
    backOverlay: true,
  ...
  // Notific: Report Default Settings: end

  // Notific: Confirm Default Settings: begin
  var confirmSettings = {
    ID: 'NotificConfirmWrap', // can not customizable
    className: 'Notific-confirm',
    width: '300px',
    zindex: 4003,
    ...
    messageMaxLength: 130,

    buttonsFontSize: '15px',
    buttonsMaxLength: 34,
    okButtonColor: 'bg-[var(--goat-black)]',
    okButtonBackground: '#5DA1E4',
    cancelButtonColor: 'bg-[var(--goat-black)]',
    cancelButtonBackground: '#a9a9a9',
  };
  // Notific: Confirm Default Settings: end

  // Notific: Loading Default Settings: begin
  var loadingSettings = {
    ID: 'NotificLoadingWrap', // can not customizable
    className: 'Notific-loading',
    zindex: 4000,
    ...
    customSvgUrl: null,
    svgSize: '80px',
    svgColor: 'rgba(233, 29, 39, 1)',
    messageID: 'NotificLoadingMessage',
    messageFontSize: '15px',
    messageMaxLength: 34,
    messageColor: 'white',
  };
  // Notific: Loading Default Settings: end

  // Notific: Block Default Settings: begin
  var blockSettings = {
    ID: 'NotificBlockWrap', // can not customizable
    querySelectorLimit: 200,
    className: 'Notific-block',
    position: 'absolute',
    zindex: 1000,
  ...
  // Notific: Block Default Settings: end

  // Notific: Console Error: begin
  var NotificConsoleError = function (title, message) {
    return console.error('%c ' + title + ' ', 'padding:2px;border-radius:20px;color:#fff;background:#ff5549', '\n' + message + NotificConsoleDocs);
  };
  // Notific: Console Error: end

  // Notific: Console Log: begin
  var NotificConsoleLog = function (title, message) {
    return console.log('%c ' + title + ' ', 'padding:2px;border-radius:20px;color:#fff;background:#26c0d3', '\n' + message + NotificConsoleDocs);
  };
  // Notific: Console Log: end

  // Notific: Check Head or Body: begin
  var NotificHeadOrBodyCheck = function (element) {
    if (!element) { element = 'head'; }
    if (window.document[element] === null) {
      NotificConsoleError('Notific', '\nNotific needs to be appended to the "<' + element + '>" element, but you called it before the "<' + element + '>" element has been created.');
      return false;
    }
    return true;
  };
      ...
      for (var prop in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, prop)) {
          ...
          if (deep && Object.prototype.toString.call(obj[prop]) === '[object Object]') {
            extended[prop] = extendNotific(extended[prop], obj[prop]);
          } else {
  ...
  // Notific: Plaintext: begin
  var NotificPlaintext = function (html) {
    var htmlPool = window.document.createElement('div');
    htmlPool.innerHTML = html;
    return htmlPool.textContent || htmlPool.innerText || '';
  };
  // Notific: Plaintext: end

  // Notific: GoogleFont: begin
  var NotificGoogleFont = function (use, family) {
    if (!window.document.getElementById('NotificQuicksand') && use && (typeof family === 'string' && family.toLocaleLowerCase('en') === 'quicksand')) {
      // check doc head
      if (!NotificHeadOrBodyCheck('head')) { return false; }

      // google fonts dns prefetch: begin
      var dns = '<link id="NotificGoogleDNS" rel="dns-prefetch" href="//fonts.googleapis.com" />';
      ...
      var font = '<link id="NotificQuicksand" href="https://fonts.googleapis.com/css?family=Quicksand:300,400,500,700&amp;subset=latin-ext" rel="stylesheet" />';
  ...
  // Notific: GoogleFont: end

  // Notific: Report SVG Success: begin
  var NotificReportSvgSuccess = function (width, color) {
    if (!width) { width = '110px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Report SVG Success: end

  // Notific: Report SVG Failure: begin
  var NotificReportSvgFailure = function (width, color) {
    if (!width) { width = '110px'; }
  ...
  // Notific: Report SVG Failure: end

  // Notific: Report SVG Warning: begin
  var NotificReportSvgWarning = function (width, color) {
    if (!width) { width = '110px'; }
  ...
  // Notific: Report SVG Warning: end

  // Notific: Report SVG Info: begin
  var NotificReportSvgInfo = function (width, color) {
    if (!width) { width = '110px'; }
  ...
  // Notific: Report SVG Info: end

  // Notific: Indicator SVG standard: begin
  var NotificIndicatorSvgStandard = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG standard: end

  // Notific: Indicator SVG hourglass: begin
  var NotificIndicatorSvgHourglass = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG hourglass: end

  // Notific: Indicator SVG circle: begin
  var NotificIndicatorSvgCircle = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG circle: end

  // Notific: Indicator SVG arrows: begin
  var NotificIndicatorSvgArrows = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG arrows: end

  // Notific: Indicator SVG dots: begin
  var NotificIndicatorSvgDots = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG dots: end

  // Notific: Indicator SVG pulse: begin
  var NotificIndicatorSvgPulse = function (width, color) {
    if (!width) { width = '60px'; }
    if (!color) { color = '#5DA1E4'; }
  ...
  // Notific: Indicator SVG pulse: end

  // Notific: Indicator SVG Notific: begin
  var NotificIndicatorSvgNotific = function (width, white, green) {
    if (!width) { width = '60px'; }
    if (!white) { white = 'bg-[var(--goat-black)]'; }
    if (!green) { green = '#5DA1E4'; }
    var NotificIcon = '<svg xmlns="http://www.w3.org/2000/svg" id="NXLoadingNotificLib" width="' + width + '" height="' + width + '" fill-rule="evenodd" clip-rule="evenodd" image-rendering="optimizeQuality" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" version="1.1" viewBox="0 0 200 200" xml:space="preserve"><defs><style>@keyframes Notific-n{0%{stroke-dashoffset:1000}to{stroke-dashoffset:0}}@keyframes Notific-x{0%{stroke-dashoffset:1000}to{stroke-dashoffset:0}}@keyframes Notific-dot{0%,to{stroke-width:0}50%{stroke-width:12}}.nx-icon-line{stroke:' + white + ';stroke-width:12;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:22;fill:none}</style></defs><path d="M47.97 135.05a6.5 6.5 0 1 1 0 13 6.5 6.5 0 0 1 0-13z" style="animation-name:Notific-dot;animation-timing-function:ease-in-out;animation-duration:1.25s;animation-iteration-count:infinite;animation-direction:normal" fill="' + green + '" stroke="' + green + '" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="22" stroke-width="12"/><path class="nx-icon-line" d="M10.14 144.76V87.55c0-5.68-4.54-41.36 37.83-41.36 42.36 0 37.82 35.68 37.82 41.36v57.21" style="animation-name:Notific-n;animation-timing-function:linear;animation-duration:2.5s;animation-delay:0s;animation-iteration-count:infinite;animation-direction:normal" stroke-dasharray="500"/><path class="nx-icon-line" d="M115.06 144.49c24.98-32.68 49.96-65.35 74.94-98.03M114.89 46.6c25.09 32.58 50.19 65.17 75.29 97.75" style="animation-name:Notific-x;animation-timing-function:linear;animation-duration:2.5s;animation-delay:.2s;animation-iteration-count:infinite;animation-direction:normal" stroke-dasharray="500"/></svg>';
    return NotificIcon;
  };
  // Notific: Indicator SVG Notific: end

  // Notific: Notify Single: begin
  var notifyElmCount = 0;
  var notifyElmCountOnlyCallback = 0;
  var NotificNotify = function (message, callbackOrOptions, options, staticType) {
    // check doc body
    if (!NotificHeadOrBodyCheck('body')) { return false; }
    ...
    var newNotifySettingsBackup = extendNotific(true, newNotifySettings, {});
    ...
    if ((typeof callbackOrOptions === 'object' && !Array.isArray(callbackOrOptions)) || (typeof options === 'object' && !Array.isArray(options))) {
      ...
      newNotifySettings = extendNotific(true, newNotifySettings, newOptions);
    }
    ...
    if (typeof message !== 'string') {
      message = 'Notific ' + staticType;
    }
    // if no message: end

    // if plainText true = HTML tags not allowed: begin
    if (newNotifySettings.plainText) {
      message = NotificPlaintext(message); // message plain text
    }
    ...
    if (!newNotifySettings.plainText && message.length > newNotifySettings.messageMaxLength) {
      ...
      newNotifySettings = extendNotific(true, newNotifySettings, { closeButton: true, messageMaxLength: 100 });
    ...
    if (newNotifySettings.useIcon) {
      ...
      // use Notific icon
        ...
        if (staticType === 'Success') {  // success
          svgIcon = '<svg class="nmi" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="40px" height="40px" version="1.1" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"viewBox="0 0 40 40"xmlns:xlink="http://www.w3.org/1999/xlink"><defs><style type="text/css">#Notific-Icon-Success{fill:' + theType.NotificIconColor + '}</style></defs><g><path id="Notific-Icon-Success" d="M20 0c11.03,0 20,8.97 20,20 0,11.03 -8.97,20 -20,20 -11.03,0 -20,-8.97 -20,-20 0,-11.03 8.97,-20 20,-20zm0 37.98c9.92,0 17.98,-8.06 17.98,-17.98 0,-9.92 -8.06,-17.98 -17.98,-17.98 -9.92,0 -17.98,8.06 -17.98,17.98 0,9.92 8.06,17.98 17.98,17.98zm-2.4 -13.29l11.52 -12.96c0.37,-0.41 1.01,-0.45 1.42,-0.08 0.42,0.37 0.46,1 0.09,1.42l-12.16 13.67c-0.19,0.22 -0.46,0.34 -0.75,0.34 -0.23,0 -0.45,-0.07 -0.63,-0.22l-7.6 -6.07c-0.43,-0.35 -0.5,-0.99 -0.16,-1.42 0.35,-0.43 0.99,-0.5 1.42,-0.16l6.85 5.48z"/></g></svg>';
        } else if (staticType === 'Failure') { // failure
          svgIcon = '<svg class="nmi" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="40px" height="40px" version="1.1" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"viewBox="0 0 40 40"xmlns:xlink="http://www.w3.org/1999/xlink"><defs><style type="text/css">#Notific-Icon-Failure{fill:' + theType.NotificIconColor + '}</style></defs><g><path id="Notific-Icon-Failure" d="M20 0c11.03,0 20,8.97 20,20 0,11.03 -8.97,20 -20,20 -11.03,0 -20,-8.97 -20,-20 0,-11.03 8.97,-20 20,-20zm0 37.98c9.92,0 17.98,-8.06 17.98,-17.98 0,-9.92 -8.06,-17.98 -17.98,-17.98 -9.92,0 -17.98,8.06 -17.98,17.98 0,9.92 8.06,17.98 17.98,17.98zm1.42 -17.98l6.13 6.12c0.39,0.4 0.39,1.04 0,1.43 -0.19,0.19 -0.45,0.29 -0.71,0.29 -0.27,0 -0.53,-0.1 -0.72,-0.29l-6.12 -6.13 -6.13 6.13c-0.19,0.19 -0.44,0.29 -0.71,0.29 -0.27,0 -0.52,-0.1 -0.71,-0.29 -0.39,-0.39 -0.39,-1.03 0,-1.43l6.13 -6.12 -6.13 -6.13c-0.39,-0.39 -0.39,-1.03 0,-1.42 0.39,-0.39 1.03,-0.39 1.42,0l6.13 6.12 6.12 -6.12c0.4,-0.39 1.04,-0.39 1.43,0 0.39,0.39 0.39,1.03 0,1.42l-6.13 6.13z"/></g></svg>';
        } else if (staticType === 'Warning') { // warning
          svgIcon = '<svg class="nmi" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="40px" height="40px" version="1.1" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"viewBox="0 0 40 40"xmlns:xlink="http://www.w3.org/1999/xlink"><defs><style type="text/css">#Notific-Icon-Warning{fill:' + theType.NotificIconColor + '}</style></defs><g><path id="Notific-Icon-Warning" d="M21.91 3.48l17.8 30.89c0.84,1.46 -0.23,3.25 -1.91,3.25l-35.6 0c-1.68,0 -2.75,-1.79 -1.91,-3.25l17.8 -30.89c0.85,-1.47 2.97,-1.47 3.82,0zm16.15 31.84l-17.8 -30.89c-0.11,-0.2 -0.41,-0.2 -0.52,0l-17.8 30.89c-0.12,0.2 0.05,0.4 0.26,0.4l35.6 0c0.21,0 0.38,-0.2 0.26,-0.4zm-19.01 -4.12l0 -1.05c0,-0.53 0.42,-0.95 0.95,-0.95 0.53,0 0.95,0.42 0.95,0.95l0 1.05c0,0.53 -0.42,0.95 -0.95,0.95 -0.53,0 -0.95,-0.42 -0.95,-0.95zm0 -4.66l0 -13.39c0,-0.52 0.42,-0.95 0.95,-0.95 0.53,0 0.95,0.43 0.95,0.95l0 13.39c0,0.53 -0.42,0.96 -0.95,0.96 -0.53,0 -0.95,-0.43 -0.95,-0.96z"/></g></svg>';
        } else if (staticType === 'Info') { // info
          svgIcon = '<svg class="nmi" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="40px" height="40px" version="1.1" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"viewBox="0 0 40 40"xmlns:xlink="http://www.w3.org/1999/xlink"><defs><style type="text/css">#Notific-Icon-Info{fill:' + theType.NotificIconColor + '}</style></defs><g><path id="Notific-Icon-Info" d="M20 0c11.03,0 20,8.97 20,20 0,11.03 -8.97,20 -20,20 -11.03,0 -20,-8.97 -20,-20 0,-11.03 8.97,-20 20,-20zm0 37.98c9.92,0 17.98,-8.06 17.98,-17.98 0,-9.92 -8.06,-17.98 -17.98,-17.98 -9.92,0 -17.98,8.06 -17.98,17.98 0,9.92 8.06,17.98 17.98,17.98zm-0.99 -23.3c0,-0.54 0.44,-0.98 0.99,-0.98 0.55,0 0.99,0.44 0.99,0.98l0 15.86c0,0.55 -0.44,0.99 -0.99,0.99 -0.55,0 -0.99,-0.44 -0.99,-0.99l0 -15.86zm0 -5.22c0,-0.55 0.44,-0.99 0.99,-0.99 0.55,0 0.99,0.44 0.99,0.99l0 1.09c0,0.54 -0.44,0.99 -0.99,0.99 -0.55,0 -0.99,-0.45 -0.99,-0.99l0 -1.09z"/></g></svg>';
        } else {
          svgIcon = '';
        }
    ...
    newNotifySettings = extendNotific(true, newNotifySettings, newNotifySettingsBackup);

  };
  // Notific: Notify Single: end

  // Notific: Report Single: begin
  var NotificReport = function (title, message, buttonText, callbackOrOptions, options, staticType) {
    // check doc body
    if (!NotificHeadOrBodyCheck('body')) { return false; }
    ...
    if ((typeof callbackOrOptions === 'object' && !Array.isArray(callbackOrOptions)) || (typeof options === 'object' && !Array.isArray(options))) {
      ...
      newReportSettingsBackup = extendNotific(true, newReportSettings, {});

      // extend new settings with the options
      newReportSettings = extendNotific(true, newReportSettings, newOptions);
    }
    ...
    if (typeof title !== 'string') { title = 'Notific ' + staticType; }
    if (typeof message !== 'string') {
    ...
    if (newReportSettings.plainText) {
      title = NotificPlaintext(title);
      message = NotificPlaintext(message);
      buttonText = NotificPlaintext(buttonText);
    }
    // if plainText true = HTML tags not allowed: end
    ...
    var svgIcon = '';
    if (staticType === 'Success') {
      svgIcon = NotificReportSvgSuccess(newReportSettings.svgSize, theType.svgColor);
    } else if (staticType === 'Failure') {
      svgIcon = NotificReportSvgFailure(newReportSettings.svgSize, theType.svgColor);
    } else if (staticType === 'Warning') {
      svgIcon = NotificReportSvgWarning(newReportSettings.svgSize, theType.svgColor);
    } else if (staticType === 'Info') {
      svgIcon = NotificReportSvgInfo(newReportSettings.svgSize, theType.svgColor);
    }
    // svg icon: end
    ...
    // extend new settings with the backup settings
    newReportSettings = extendNotific(true, newReportSettings, newReportSettingsBackup);

  };
  // Notific: Report Single: end

  // Notific: Confirm Single: begin
  var NotificConfirm = function (title, message, okButtonText, cancelButtonText, okButtonCallback, cancelButtonCallback, options, hasValidation, answer) {
    // check doc body
    if (!NotificHeadOrBodyCheck('body')) { return false; }

    // if not initialized pretend like init
    if (!newConfirmSettings) {
      Notific.Confirm.Init({});
    }

    // create a backup for new settings
    var newConfirmSettingsBackup = extendNotific(true, newConfirmSettings, {});

    // check the "options": begin
    if (typeof options === 'object' && !Array.isArray(options)) {
      // extend new settings with the new options
      newConfirmSettings = extendNotific(true, newConfirmSettings, options);
    }
    // check the "options": end

    // check the arguments: begin
    if (typeof title !== 'string') { title = 'Notific Confirm'; }
    ...
    if (newConfirmSettings.plainText) {
      title = NotificPlaintext(title);
      message = NotificPlaintext(message);
      okButtonText = NotificPlaintext(okButtonText);
      cancelButtonText = NotificPlaintext(cancelButtonText);
    }
    // if plainText true => HTML tags not allowed: end
    ...
    // extend new settings with the backup settings
    newConfirmSettings = extendNotific(true, newConfirmSettings, newConfirmSettingsBackup);
  };
  // Notific: Confirm Single: end

  // Notific: Loading Single: begin
  var NotificLoading = function (messageOrOptions, options, iconType, display, delay) {
    // check doc body
    if (!NotificHeadOrBodyCheck('body')) { return false; }

    // if not initialized pretend like init
    if (!newLoadingSettings) {
      Notific.Loading.Init({});
    }

    // create a backup for new settings
    var newLoadingSettingsBackup = extendNotific(true, newLoadingSettings, {});
    ...
    if ((typeof messageOrOptions === 'object' && !Array.isArray(messageOrOptions)) || (typeof options === 'object' && !Array.isArray(options))) {
      ...
      // extend new settings with the new options
      newLoadingSettings = extendNotific(true, newLoadingSettings, newOptions);
    }
    ...
    if (display) {
      ...
      if (message.toString().length > newLoadingSettings.messageMaxLength) {
        message = NotificPlaintext(message).toString().substring(0, newLoadingSettings.messageMaxLength) + '...';
      } else {
        message = NotificPlaintext(message).toString();
      }
      var messageHTML = '';
      if (message.length > 0) {
      ...
      if (iconType === 'standard') {
        svgIcon = NotificIndicatorSvgStandard(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'hourglass') {
        svgIcon = NotificIndicatorSvgHourglass(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'circle') {
        svgIcon = NotificIndicatorSvgCircle(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'arrows') {
        svgIcon = NotificIndicatorSvgArrows(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'dots') {
        svgIcon = NotificIndicatorSvgDots(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'pulse') {
        svgIcon = NotificIndicatorSvgPulse(newLoadingSettings.svgSize, newLoadingSettings.svgColor);
      } else if (iconType === 'custom' && newLoadingSettings.customSvgUrl !== null) {
        svgIcon = '<img class="custom-loading-icon" width="' + newLoadingSettings.svgSize + '" height="' + newLoadingSettings.svgSize + '" src="' + newLoadingSettings.customSvgUrl + '" alt="Notific">';
      } else if (iconType === 'custom' && newLoadingSettings.customSvgUrl == null) {
        NotificConsoleError('Notific Error', 'You have to set a static SVG url to "customSvgUrl" option to use Loading Custom.');
        return false;
      } else if (iconType === 'Notific') {
        svgIcon = NotificIndicatorSvgNotific(newLoadingSettings.svgSize, 'bg-[var(--goat-black)]', '#5DA1E4');
      }
    ...
    // extend new settings with the backup settings
    newLoadingSettings = extendNotific(true, newLoadingSettings, newLoadingSettingsBackup);
  };
  // Notific: Loading Single: end

  // Notific: Loading Change Message: begin
  var NotificLoadingChange = function (newMessage) {
    ...
    if (messageWrap) {
      ...
      if (newMessage.length > 0) {
        ...
        if (newMessage.length > newLoadingSettings.messageMaxLength) {
          newMessage = NotificPlaintext(newMessage).substring(0, newLoadingSettings.messageMaxLength) + '...';
        } else {
          newMessage = NotificPlaintext(newMessage);
        }
        // max length: end
      ...
      // if no message
        ...
        NotificConsoleError('Notific Error', 'Where is the new message?');
      }
    }
  };
  // Notific: Loading Change Message: end

  // Notific: Block or Unblock Element: begin
  var blockElmCount = 0;
  var NotificBlockUnblock = function (block, selector, iconType, messageOrOptions, options, delay) {
    ...
    if (selectorIsNotValid) {
      NotificConsoleError('Notific Error', 'The selector parameter must be a String and matches a specified CSS selector(s).');
      return false;
    }
    ...
    if (getSelector.length < 1) {
      NotificConsoleError('Notific Error', 'You called the "Notific.Block..." function with "' + selector + '" selector, but there is no such element(s) in the document.');
      return false;
    }
    ...
    // create a backup for new settings
    var newBlockSettingsBackup = extendNotific(true, newBlockSettings, {});
    ...
    if ((typeof messageOrOptions === 'object' && !Array.isArray(messageOrOptions)) || (typeof options === 'object' && !Array.isArray(options))) {
      ...
      // extend new settings with the new options
      newBlockSettings = extendNotific(true, newBlockSettings, newOptions);
    }
    // check "messageOrOptions" and "options": end
    ...
    var blockClassName = 'Notific-block';
        ...
        if (eachBlockElement.length < 1) {
          ...
          if (iconType) {
            if (iconType === 'hourglass') {
              icon = NotificIndicatorSvgHourglass(newBlockSettings.svgSize, newBlockSettings.svgColor);
            } else if (iconType === 'circle') {
              icon = NotificIndicatorSvgCircle(newBlockSettings.svgSize, newBlockSettings.svgColor);
            } else if (iconType === 'arrows') {
              icon = NotificIndicatorSvgArrows(newBlockSettings.svgSize, newBlockSettings.svgColor);
            } else if (iconType === 'dots') {
              icon = NotificIndicatorSvgDots(newBlockSettings.svgSize, newBlockSettings.svgColor);
            } else if (iconType === 'pulse') {
              icon = NotificIndicatorSvgPulse(newBlockSettings.svgSize, newBlockSettings.svgColor);
            } else {
              icon = NotificIndicatorSvgStandard(newBlockSettings.svgSize, newBlockSettings.svgColor);
            }
          }
          ...
          if (message.length > 0) {
            if (message.length > newBlockSettings.messageMaxLength) {
              message = NotificPlaintext(message).substring(0, newBlockSettings.messageMaxLength) + '...';
            } else {
              message = NotificPlaintext(message);
            }
          ...
          // block element: begin
          blockElmCount++;
          var NotificBlockWrap = window.document.createElement('div');
          NotificBlockWrap.id = blockSettings.ID + '-' + blockElmCount;
          NotificBlockWrap.className = blockClassName + '-wrap' + (newBlockSettings.cssAnimation ? ' with-animation' : '');
          NotificBlockWrap.style.position = newBlockSettings.position;
          NotificBlockWrap.style.zIndex = newBlockSettings.zindex;
          NotificBlockWrap.style.background = newBlockSettings.backgroundColor;
          NotificBlockWrap.style.animationDuration = newBlockSettings.cssAnimationDuration + 'ms';
          NotificBlockWrap.style.fontFamily = '"' + newBlockSettings.fontFamily + '", ' + defaultFontFamily;
          NotificBlockWrap.style.display = 'flex';
          NotificBlockWrap.style.flexWrap = 'wrap';
          NotificBlockWrap.style.flexDirection = 'column';
          NotificBlockWrap.style.alignItems = 'center';
          NotificBlockWrap.style.justifyContent = 'center';
          // block element: end

          // block element rtl: begin
          if (newBlockSettings.rtl) {
            NotificBlockWrap.setAttribute('dir', 'rtl');
            NotificBlockWrap.classList.add('rtl-on');
          }
          // block element rtl: end

          // block element data: begin
          NotificBlockWrap.innerHTML = iconElement + messageElement;
          ...
          // append
          eachSelector.appendChild(NotificBlockWrap);
    ...
    // unblock/remove
      ...
      var removeClassBlockElements = function (eachBlockElement) {
        ...
        // not exist
          ...
          NotificConsoleLog('Notific Info', '"Notific.Block.Remove();" function called with "' + selector + '" selector, but this selector does not have a "Notific.Block..." element to remove.');
        }
      };
      // Step 2A => Remove each block element: begin
    ...
    // extend new settings with the backup settings
    newBlockSettings = extendNotific(true, newBlockSettings, newBlockSettingsBackup);
  };
  // Notific: Block or Unblock Element: end
  ...
  var Notific = {
    ...
    Notify: {
      ...
      Init: function (userNotifyOpt) {
        // extend options
        newNotifySettings = extendNotific(true, notifySettings, userNotifyOpt);
        // use GoogleFonts if "Quicksand"
        NotificGoogleFont(newNotifySettings.useGoogleFont, newNotifySettings.fontFamily);
      ...
      Merge: function (userNotifyExtend) {
        ...
        if (newNotifySettings) {
          newNotifySettings = extendNotific(true, newNotifySettings, userNotifyExtend);
        }
        // initialize first
        else {
          NotificConsoleError('Notific Error', 'You have to initialize the Notify module before call Merge function.');
          return false;
        }
      },
      // Display Notification: Success
      Success: function (message, callbackOrOptions, options) {
        NotificNotify(message, callbackOrOptions, options, 'Success');
      },
      // Display Notification: Failure
      Failure: function (message, callbackOrOptions, options) {
        NotificNotify(message, callbackOrOptions, options, 'Failure');
      },
      // Display Notification: Warning
      Warning: function (message, callbackOrOptions, options) {
        NotificNotify(message, callbackOrOptions, options, 'Warning');
      },
      // Display Notification: Info
      Info: function (message, callbackOrOptions, options) {
        NotificNotify(message, callbackOrOptions, options, 'Info');
      },
    },
    // Notify: end
    ...
    Report: {
      ...
      Init: function (userReportOpt) {
        // extend options
        newReportSettings = extendNotific(true, reportSettings, userReportOpt);
        // use GoogleFonts if "Quicksand"
        NotificGoogleFont(newReportSettings.useGoogleFont, newReportSettings.fontFamily);
        // internal css if exist
        NotificInternalCSS();
      },
      // Merge First Init
      Merge: function (userReportExtend) {
        // if initialized already
        if (newReportSettings) {
          newReportSettings = extendNotific(true, newReportSettings, userReportExtend);
        }
        // initialize first
        else {
          NotificConsoleError('Notific Error', 'You have to initialize the Report module before call Merge function.');
          return false;
        }
      },
      // Display Report: Success
      Success: function (title, message, buttonText, callbackOrOptions, options) {
        NotificReport(title, message, buttonText, callbackOrOptions, options, 'Success');
      },
      // Display Report: Failure
      Failure: function (title, message, buttonText, callbackOrOptions, options) {
        NotificReport(title, message, buttonText, callbackOrOptions, options, 'Failure');
      },
      // Display Report: Warning
      Warning: function (title, message, buttonText, callbackOrOptions, options) {
        NotificReport(title, message, buttonText, callbackOrOptions, options, 'Warning');
      },
      // Display Report: Info
      Info: function (title, message, buttonText, callbackOrOptions, options) {
        NotificReport(title, message, buttonText, callbackOrOptions, options, 'Info');
      },
    },
    // Report: end
    ...
    Confirm: {
      ...
      Init: function (userConfirmOpt) {
        // extend options
        newConfirmSettings = extendNotific(true, confirmSettings, userConfirmOpt);
        // use GoogleFonts if "Quicksand"
        NotificGoogleFont(newConfirmSettings.useGoogleFont, newConfirmSettings.fontFamily);
        // internal css if exist
        NotificInternalCSS();
      },
      // Merge First Init
      Merge: function (userConfirmExtend) {
        // if initialized already
        if (newConfirmSettings) {
          newConfirmSettings = extendNotific(true, newConfirmSettings, userConfirmExtend);
        }
        // initialize first
        else {
          NotificConsoleError('Notific Error', 'You have to initialize the Confirm module before call Merge function.');
          return false;
        }
      },
      // Display Confirm: Show
      Show: function (title, message, okText, cancelText, okCallback, cancelCallback, options) {
        NotificConfirm(title, message, okText, cancelText, okCallback, cancelCallback, options, false, false);
      },
      // Display Confirm: Ask
      Ask: function (title, question, answer, okText, cancelText, okCallback, cancelCallback, options) {
        NotificConfirm(title, question, okText, cancelText, okCallback, cancelCallback, options, true, answer);
      },
    },
    // Confirm: end
    ...
    Loading: {
      ...
      Init: function (userLoadingOpt) {
        // extend options
        newLoadingSettings = extendNotific(true, loadingSettings, userLoadingOpt);
        // use GoogleFonts if "Quicksand"
        NotificGoogleFont(newLoadingSettings.useGoogleFont, newLoadingSettings.fontFamily);
        // internal css if exist
        NotificInternalCSS();
      },
      // Merge First Init
      Merge: function (userLoadingExtend) {
        // if initialized already
        if (newLoadingSettings) {
          newLoadingSettings = extendNotific(true, newLoadingSettings, userLoadingExtend);
        }
        // initialize first
        else {
          NotificConsoleError('Notific Error', 'You have to initialize the Loading module before call Merge function.');
          return false;
        }
      },
      // Display Loading: Standard
      Standard: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'standard', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Hourglass
      Hourglass: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'hourglass', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Circle
      Circle: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'circle', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Arrows
      Arrows: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'arrows', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Dots
      Dots: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'dots', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Pulse
      Pulse: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'pulse', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Custom
      Custom: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'custom', true, 0); // true => show && 0 => delay
      },
      // Display Loading: Notific
      Notific: function (messageOrOptions, options) {
        NotificLoading(messageOrOptions, options, 'Notific', true, 0); // true => show && 0 => delay
      },
      // Remove Loading
      Remove: function (delay) {
        if (typeof delay !== 'number') { delay = 0; }
        NotificLoading(false, false, false, false, delay); // false => hide/remove
      },
      // Change The Message
      Change: function (newMessage) {
        NotificLoadingChange(newMessage);
      },
    },
    // Loading: end
    ...
    Block: {
      ...
      Init: function (userBlockOpt) {
        // extend options
        newBlockSettings = extendNotific(true, blockSettings, userBlockOpt);
        // use GoogleFonts if "Quicksand"
        NotificGoogleFont(newBlockSettings.useGoogleFont, newBlockSettings.fontFamily);
        // internal css if exist
        NotificInternalCSS();
      },
      ...
      Merge: function (userBlockExtend) {
        // if initialized already
        if (newBlockSettings) {
          newBlockSettings = extendNotific(true, newBlockSettings, userBlockExtend);
        }
        // initialize first
        else {
          NotificConsoleError('Notific Error', 'You have to initialize the "Notific.Block" module before call Merge function.');
          return false;
        }
      },
      // Display Block: Standard
      Standard: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'standard', messageOrOptions, options); // true => show
      },
      // Display Block: Hourglass
      Hourglass: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'hourglass', messageOrOptions, options); // true => show
      },
      // Display Block: Circle
      Circle: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'circle', messageOrOptions, options); // true => show
      },
      // Display Block: Arrows
      Arrows: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'arrows', messageOrOptions, options); // true => show
      },
      // Display Block: Dots
      Dots: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'dots', messageOrOptions, options); // true => show
      },
      // Display Block: Pulse
      Pulse: function (selector, messageOrOptions, options) {
        NotificBlockUnblock(true, selector, 'pulse', messageOrOptions, options); // true => show
      },
      // Remove Block
      Remove: function (selector, delay) {
        if (typeof delay !== 'number') { delay = 0; }
        NotificBlockUnblock(false, selector, false, false, false, delay); // false => hide/remove
      },
    },
    // Block: end
  };
  ...
  // Notific: Return: begin
  return {
    Notify: Notific.Notify,
    Report: Notific.Report,
    Confirm: Notific.Confirm,
    Loading: Notific.Loading,
    Block: Notific.Block,
  };
  // Notific: Return: end
});
