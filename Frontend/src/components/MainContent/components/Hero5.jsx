...
const Hero5 = () => {
    ...
    // Preload images when component mounts (in background)
    useEffect(() => {
        const preloadImages = async () => {
            // Preload all images in background
            await imagePreloader.preloadImages([...portraitImages, ...landscapeImages]);
            setImagesReady(true);
        };
        preloadImages();
    }, []);
    ...
    return (
        <div className="p-5 text-center mt-6" id="hero5">
            {/* Title */}
            <img src={GoatWallText} alt="Goat Wall" loading="lazy" className="mx-auto mb-8" />
                ...
                <div className="w-full h-[456px] border-[var(--goat-red)] border rounded-2xl overflow-hidden relative">
                    {landscapeImages.map((img, i) => (
                        <img
                            ...
                            alt={`landscape ${i + 1}`}
                            loading="eager"
                            className={`absolute top-0 left-0 w-full h-full object-cover rounded-2xl transition-opacity duration-[2000ms] ease-in-out ${i === landscapeIndex ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                            style={{
                                willChange: i === landscapeIndex || i === (landscapeIndex + 1) % landscapeImages.length || i === (landscapeIndex - 1 + landscapeImages.length) % landscapeImages.length ? "opacity" : "auto"
                            }}
                        />
                    ))}
                </div>
                ...
                <div className="hidden lg:block w-[30%] h-[456px] border-[var(--goat-red)] border rounded-2xl overflow-hidden relative">
                    {portraitImages.map((img, i) => (
                        <img
                            ...
                            alt={`portrait ${i + 1}`}
                            loading="eager"
                            className={`absolute top-0 left-0 w-full h-full object-cover rounded-2xl transition-opacity duration-[2000ms] ease-in-out ${i === portraitIndex ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                            style={{
                                willChange: i === portraitIndex || i === (portraitIndex + 1) % portraitImages.length || i === (portraitIndex - 1 + portraitImages.length) % portraitImages.length ? "opacity" : "auto"
                            }}
                        />
                    ))}
                </div>
